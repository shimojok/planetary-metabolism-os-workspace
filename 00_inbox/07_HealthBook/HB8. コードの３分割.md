...
了解しました。Pydroidでコードをコピーする際の問題ですね。コードを分割して提供します。

## 解決策1: コードを複数のファイルに分割

### ファイル1: `demo_database.py`
```python
"""
HealthBookデモ - データベース部分
"""
...
### ファイル2: `demo_engine.py`
```python
"""
HealthBookデモ - コアエンジン部分
"""
...
### ファイル3: `demo_main.py` （メイン実行ファイル）
```python
"""
HealthBookデモ - メイン実行ファイル
Pydroidでこのファイルを実行してください
"""

import sys

# 他のファイルからインポート
from demo_engine import HealthBookDemoEngine
from demo_database import DemoDatabase

def simple_demo():
    """シンプルなデモを実行"""
    print("="*50)
    print("HealthBook 簡易デモ")
    print("="*50)
    print("\n3つの質問に答えるだけで健康分析ができます。\n")
    ...
    engine.user_responses = {
        1: answers[0] if len(answers) > 0 else False,
        3: answers[1] if len(answers) > 1 else False,
        5: answers[2] if len(answers) > 2 else False
    }
    ...
    risk_score = sum(answers) / len(answers) if answers else 0
    ...
    if engine.analysis_results:
        disease_risks = engine.analysis_results.get("disease_risks", {})
        if disease_risks:
            print("\n🔍 詳細リスク:")
            for disease, risk_info in list(disease_risks.items())[:2]:
                print(f"  • {disease}: {risk_info['level']} ({risk_info['score']:.0%})")
    ...
    if answers[1]:  # 塩辛い物好き
        print("  • 塩分控えめを心がけましょう")
    if answers[2]:  # ストレス多い
        print("  • ストレス解消法を見つけましょう")
    
    print("\n" + "="*50)
    
    # 詳細分析オプション
    choice = input("\n詳細分析を行いますか？ (はい/いいえ): ").strip().lower()
    if choice in ['はい', 'yes', 'y', '1']:
        full_demo()

def full_demo():
    """完全なデモを実行"""
    print("\n" + "="*50)
    print("HealthBook 完全デモ")
    print("="*50)
...
def show_info():
    ...
    print(f"  疾病: {len(DemoDatabase.get_disease_matrix())}種類")
...
もし分割も難しい場合は、この超シンプルな単一ファイル版を使ってください：
...
def simple_health_check():
    ...
    for i, q in enumerate(questions, 1):
        while True:
            ans = input(f"{i}. {q} (はい/いいえ): ").lower()
            if ans in ['はい', 'yes', 'y', '1']:
                score += 1
                break
            elif ans in ['いいえ', 'no', 'n', '0']:
                break
            else:
                print("「はい」か「いいえ」で答えてください")
    ...
    if score >= 1:
        ...
        print("• 塩分控えめを心がけましょう")
        print("• ストレス解消法を見つけましょう")
        print("• 色の濃い野菜をたくさん食べましょう")
    else:
        print("• 現在の生活習慣を維持しましょう")
        print("• バランスの良い食事を続けましょう")
    
    print("\n" + "="*50)
    
    # 詳細情報
    if input("\n詳細情報を見ますか？ (はい/いいえ): ").lower() in ['はい', 'yes', 'y', '1']:
        show_details()

def show_details():
    """詳細情報を表示"""
    print("\n" + "="*50)
    print("HealthBook 詳細情報")
    print("="*50)
...
### 方法B: シンプル単一ファイル版
1. Pydroidで1つのファイルを作成（例: `healthbook_simple.py`）

2. 「超シンプルな単一ファイル版」のコードをすべてコピー
...
このデモシステムを実際のシステムに拡張するには：

1. **データ拡張**: `demo_database.py` に実際の200問診項目を追加
2. **アルゴリズム改善**: `demo_engine.py` の分析ロジックを強化
3. **UI改善**: グラフィカルなインターフェースを追加
4. **クラウド連携**: Azureへのデータ保存機能を追加

まずはこのデモで基本コンセプトを体験し、必要に応じて段階的に拡張していくことをお勧めします。
